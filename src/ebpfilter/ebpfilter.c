...
int fw_dev_get_ip(char *iface, __be32 *ip)
{
	int fd;
	struct ifreq ifr;
	int ret;
...
int fw_do_nat_add(int argc, char **argv, int nat_type)
{
...
int fw_nat_add(int argc, char **argv)
{
	return fw_do_nat_add(argc, argv, 0);
}

int fw_snat_add(int argc, char **argv)
{
	return fw_do_nat_add(argc, argv, FW_NAT_SOURCE);
}

int fw_nat_show_dev(void)
{
	struct in_addr in;
	int ret, key = 0;
	int nat_fd;
...
int fw_nat_show_all(struct if_nameindex *iface, int attached)
{
	if (!attached)
		return 0;
	opts.ifindex = iface->if_index;
...
int fw_nat_show(int argc, char **argv)
{
	int ret;
...
int fw_nat_flush(int argc, char **argv)
{
	int ret, key = 0;
	int nat_fd;
	struct fw_nat_rule rule;
...
int fw_nat_help(__unused int argc, __unused char **argv)
{
